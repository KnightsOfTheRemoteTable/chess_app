module GamesHelper
  ...
  def draw_piece(x_axis, y_axis)
    ...
    if current_piece.id == selected_piece.id 
      link_to image_tag("#{current_piece.color.downcase}_#{current_piece.type.downcase}.png", id: "#{current_piece.id}", class: "selected"), piece_path(current_piece)
    else
